import type { ClientParams, Sendmail, SendmailBatch, TemplateQueryParams, TemplateBatchParams } from "../types";
import { Client } from "../client/nodeSendMail";
declare class SendMailClient extends Client {
    isUrl: string | boolean;
    constructor(options: ClientParams, clientOption?: {});
    sendMail(options: Sendmail): Promise<unknown>;
    sendBatchMail(options: SendmailBatch): Promise<unknown>;
    sendMailWithTemplate(options: TemplateQueryParams): Promise<unknown>;
    mailBatchWithTemplate(options: TemplateBatchParams): Promise<unknown>;
}
export { SendMailClient };
export type { Sendmail, SendmailBatch, TemplateQueryParams, TemplateBatchParams, ClientParams } from "../types";
